-- Add scheduled_time column to scheduled_tasks table
-- This allows users to specify what time of day their scheduled tasks should be activated
-- Default is 00:00:00 (midnight) for existing tasks

ALTER TABLE public.scheduled_tasks 
ADD COLUMN scheduled_time TIME DEFAULT '00:00:00'::TIME NOT NULL;

-- Add comment explaining the column
COMMENT ON COLUMN public.scheduled_tasks.scheduled_time IS 'Time of day when the scheduled task should be activated (in user timezone)';

-- Update the convert_card_to_scheduled_task function to include the time parameter
CREATE OR REPLACE FUNCTION public.convert_card_to_scheduled_task(
  p_card_id UUID,
  p_recurrence_type TEXT,
  p_days_of_week INTEGER[],
  p_next_occurrence_date DATE,
  p_scheduled_time TIME DEFAULT '00:00:00'::TIME
)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.convert_card_to_scheduled_task(UUID, TEXT, INTEGER[], DATE, TIME) TO authenticated;